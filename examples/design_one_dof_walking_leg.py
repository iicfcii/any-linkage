...
class OneDoFWalkingLegDesign(designer.Design):
    def plans():
        plans = topology.load()
    ...
    def __init__(self, plan_index, seed=0, device="cuda"):
        ...
        self.plan = OneDoFWalkingLegDesign.plans()[self.plan_index]
        ...
        self.p_output_d[:-2, 0] = torch.linspace(
            -50, 50, self.p_output_d.shape[0] - 2,
        )
        self.p_output_d[:, 1] = -200
        self.p_output_d[-1] = torch.tensor([25, -200 + 25])
        self.p_output_d[-2] = torch.tensor([-25, -200 + 25])
    ...
    def _eval(self):
        ...
        loss_output_error = torch.mean(
            torch.amin(
                torch.linalg.norm(
                    self.p_output_d.unsqueeze(-2) - p_output.unsqueeze(-3),
                    dim=-1,
                ),
                dim=-1
            ),
...
def main():
    if sys.argv[1] == "t":
        ...
        design = OneDoFWalkingLegDesign(plan_index)
        design.eval()
        design.plot()
        plt.show()
    ...
    if sys.argv[1] == "o":
        ...
        designer.save(design, "logs", name="one_dof_walking_leg")
        design.plot()
        plt.show()
